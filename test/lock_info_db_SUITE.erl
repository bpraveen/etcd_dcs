...
store_retrieve(_Config) ->
    store_retrieve_helper(ebox).

retrieve_not_existing(_Config) ->
    {error, not_found} = lock_info_db:retrieve(#{host => "localhost", port => 23790, key => not_existing_key}).

delete(_Config) ->
    Key = some_key,
    store_retrieve_helper(Key),
    ok = lock_info_db:delete(#{host => "localhost", port => 23790, key => Key}),
    {error, not_found} = lock_info_db:delete(#{host => "localhost", port => 23790, key => Key}).
...
store_retrieve_helper(Key) ->
    Value = #{a => 1, b => 2},
    Entry = #{host => "localhost", port => 23790, key => Key, value => Value},
    ok = lock_info_db:store(Entry),
    {ok, Value} = lock_info_db:retrieve(#{host => "localhost", port => 23790, key => Key}).
